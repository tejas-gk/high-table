...
const variants = {
    base: 'relative rounded-md p-4 w-32 h-32 max-w-[calc(100vw-1rem)] flex justify-center items-center flex-col cursor-pointer border border-dashed border-gray-400 dark:border-gray-300 transition-colors duration-200 ease-in-out',
    active: 'border-2',
    disabled:
...
const MultiFileDropzone = React.forwardRef<HTMLInputElement, InputProps>(
    (
        ...
        const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (selectedFiles) {
                const addedFiles = Array.from(selectedFiles).map<FileState>((file) => ({
                    file,
                    key: Math.random().toString(36).slice(2),
                    progress: 'PENDING',
                }));
        ...
        return (
            <div>
                <div className="flex flex-col gap-2">
                    <div>
                        {/* Main File Input */}
                        <div className="flex flex-col items-start justify-start text-xs text-gray-400">
                            <div className={`h-32 w-32 border-2 border-gray-200 border-dashed rounded-md flex justify-start items-start cursor-pointer ${dropZoneClassName}`}
                                {...getRootProps()}
                            >
                                <Plus size={32} />
                                <Input
                    ...
                    {images.map((file) => (
                        <img key={file.name} src={URL.createObjectURL(file)} alt={file.name} className="h-16 w-16 object-cover" />
                    ))}
                    {/* Selected Files */}
